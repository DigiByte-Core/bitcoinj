...
import org.digibytej.utils.ContextPropagatingThreadFactory;
...
public class Context {
    ...
    public Context(NetworkParameters params, int eventHorizon, Coin feePerKb, boolean ensureMinRequiredFee) {
        log.info("Creating digibytej {} context.", VersionMessage.DIGIBYTEJ_VERSION);
    ...
    public static Context get() {
        ...
        if (tls == null) {
            if (isStrictMode) {
                log.error("Thread is missing a digibytej context.");
                log.error("You should use Context.propagate() or a ContextPropagatingThreadFactory.");
                throw new IllegalStateException("missing context");
            }
            if (lastConstructed == null)
                throw new IllegalStateException("You must construct a Context object before using digibytej!");
            slot.set(lastConstructed);
            log.error("Performing thread fixup: you are accessing digibytej via a thread that has not had any context set on it.");
